...
class IOmanager
  def save_book(books)
    ...
    book_data = books.map do |data| { title: data.title, author: data.author }
  end
...
def save_people(person)
  ...
  person_data = person.map do |persons|
    if person.instance_of?(Teacher)
      { occupation: 'Teacher', name: persons.name, age: persons.age, specialiazation: persons.specialization, id: persons.id }
    else
      { occupation: 'Student', name: persons.name, age: persons.age, parent_permission: persons.parent_permission, id: persons.id }
    end
  end
...
def fetch_person_data
  ...
  if file.empty?
    ...
    parsed_data.map do |persons|
      ...
      when 'Student'
        student = Student.new(persons['age'], 'classroom', persons['name'], persons['parent_permission'], persons['id'].to_i)
        array.push(student)
      end
      end
  end
  ...
  rentals_data = rentals.map do |rental|
    ...
    begin
      if rental.persons.specialiazation
        obj.store('person', 
        {id: rental.persons.id, age: rental.persons.age, name: rental.persons.name, parent_permission: rental.persons.parent_permission})
      end
    rescue standardError
      obj.store('person',
      { id: rental.person.id, age: rental.persons.age, name: rental.persons.name, parent_permission: rental.persons.parent_permission})
    end
    obj
    end
